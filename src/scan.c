...
static unsigned int serv_select = 3; // 20080106: radio and tv as default (no service/other). 20090227: flag type vars shouldnt
                                     // be signed.
...
struct transponder *
alloc_transponder(uint32_t frequency, unsigned delsys, uint8_t polarization)
{
...
static bool
is_auto_params(struct transponder *t)
{
    switch (t->delsys) {
    case SYS_DVBT2:
...
static int
is_nearly_same_frequency(uint32_t f1, uint32_t f2, scantype_t type)
{
    uint32_t diff;
    if (f1 == f2)
        return 1;
...
int
is_different_transponder_deep_scan(struct transponder *a, struct transponder *b, int auto_allowed)
{
...
static void
copy_transponder(struct transponder *dest, struct transponder *source)
{
    struct service *s, *sd;
    struct cell *p;
    ...
    if (source->network_change.num_networks > 0) {
        ...
        for (i = 0; i < source->network_change.num_networks; i++) {
            ...
            dest->network_change.network[i].loop = calloc(
...
find_transponder_by_freq(struct transponder *tn)
{
...
static struct transponder *
find_transponder(uint16_t original_network_id, uint16_t network_id, uint16_t transport_stream_id)
{
    struct transponder *t;
    char buf[128];
...
static int
is_known_initial_transponder(struct transponder *tn, int auto_allowed)
{
    struct transponder *t;
...
void
print_transponder(char *dest, struct transponder *t)
{
    char plp_id[5];
...
static void
copy_duplicate_tp(struct transponder *t, struct transponder *t2)
{
 ...
 */
struct service *
alloc_service(struct transponder *t, uint16_t service_id)
{
...
struct service *
find_service(struct transponder *t, uint16_t service_id)
{
    struct service *s;
...
static int
find_descriptor(uint8_t tag, unsigned char const *buf, int descriptors_loop_len, unsigned char const **desc, int *desc_len)
{
...
static void
parse_descriptors(enum table_id t, unsigned char const *buf, int descriptors_loop_len, void *data, scantype_t scantype)
{
    while (descriptors_loop_len > 0) {
        unsigned char descriptor_tag = buf[0];
        unsigned char descriptor_len = buf[1] + 2;
        ...
        case service_descriptor:
            if ((t == TABLE_SDT_ACT) || (t == TABLE_SDT_OTH))
                parse_service_descriptor(buf, data, flags.fallback_input_charset, flags.codepage);
            break;
 ...
 */
em_static void
parse_pat(unsigned char const *buf, uint16_t section_length, uint16_t transport_stream_id, uint32_t section_flags)
{
    ...
    if (current_tp->transport_stream_id != transport_stream_id) {
        if (current_tp->type == SCAN_TERRESTRIAL) {
            ...
            info(
                "        %s : updating transport_stream_id: -> (%u:%u:%u)\n",
                buffer,
    ...
    while (section_length > 0) {
        ...
        if (service_id == 0) {
            if (program_number != 16)
                info(
                    "        %s: network_PID = %d (transport_stream_id %d)\n",
                    __FUNCTION__,
                    program_number,
                    transport_stream_id);
        ...
        if (!(section_flags & SECTION_FLAG_INITIAL)) {
            if (s->priv == NULL) { //  && s->pmt_pid) {  pmt_pid is by spec: 0x0010 .. 0x1FFE . see EN13818-1 p.19 Table 2-3 -
                                   //  PID table
...
parse_pmt(unsigned char const *buf, uint16_t section_length, uint16_t service_id)
{
    ...
    while (section_length > 0) {
        ...
        case iso_iec_13818_6_type_b:
            ...
            //         Used to attach a client to a continuous-feed session (CFS) or other broadcast feed. Sometimes used in
            //         pay-per-view.
            //
...
em_static void
parse_nit(unsigned char const *buf, uint16_t section_length, uint8_t table_id, uint16_t network_id)
{
    char buffer[128];
    ...
    while (section_length > 6) {
        ...
        if ((current_tp->original_network_id == original_network_id) &&
            (current_tp->transport_stream_id == transport_stream_id) && (table_id == TABLE_NIT_ACT)) {
        ...
        if (t != NULL) {
            ...
            if (table_id == TABLE_NIT_ACT && flags.delete_duplicate_transponders) {
                ...
                if (update_pids) {
                    ...
                    if ((t->original_network_id != original_network_id) || (t->network_id != network_id) ||
                        (t->transport_stream_id != transport_stream_id)) {
                        print_transponder(buffer, t);
                        info(
                            "        %s : updating tp ids -> (%u:%u:%u)\n",
                            buffer,
                            original_network_id,
                            network_id,
                            transport_stream_id);
                ...
                if (is_different_transponder_deep_scan(t, &tn, 0) &&
                    ((!t->locks_with_params) || is_auto_params(t))) { // ||
                                                                      // t->source
                                                                      // !=
                                                                      // tn.source)
                                                                      // {
                        ...
                        for (fi = (t->cells)->first; fi; fi = fi->next) {
                            ...
                            for (n = 0; n < fi->num_transposers; n++) {
                                verbose(
                                    ...
                                    n,
                                    fi->transposers[n].transposer_frequency);
                            }
                        }
                    }
                    if (verbosity > 1)
...
em_static void
parse_sdt(unsigned char const *buf, uint16_t section_length, uint16_t transport_stream_id)
{
    hexdump(__FUNCTION__, buf, section_length);
...
em_static void
parse_psip_descriptors(struct service *s, unsigned char const *buf, int len)
{
...
em_static void
parse_psip_vct(unsigned char const *buf, uint16_t section_length, uint8_t table_id, uint16_t transport_stream_id)
{
    (void)section_length;
    (void)table_id;
...
static int
get_bit(uint8_t *bitfield, int bit)
{
    return (bitfield[bit / 8] >> (bit % 8)) & 1;
}

static void
set_bit(uint8_t *bitfield, int bit)
{
 ...
 */
static int
parse_section(struct section_buf *s)
{
    unsigned char const *buf = s->buf;
    uint8_t table_id;
    ...
    if (!crc_check(&buf[0], section_length + 12)) {
        ...
        if (s->timeout < slow_rep_rate) {
            info(
                "increasing filter timeout to %d secs (pid:%d table_id:%d table_id_ext:%d).\n",
                slow_rep_rate,
                s->pid,
                s->table_id,
                s->table_id_ext);
    ...
    section_version_number = (buf[5] >> 1) & 0x1f; // p.version_number = getBits (b, 0, 42, 5); -> 40 + 1 -> 5 bit weit? ->
    ...
    if (!get_bit(s->section_done, section_number)) {
        ...
        case TABLE_NIT_ACT:
        case TABLE_NIT_OTH:
            verbose("NIT(%s TS, network_id %d (0x%04x) )\n", table_id == 0x40 ? "actual" : "other", table_id_ext, table_id_ext);
...
static int
read_sections(struct section_buf *s)
{
...
static int
start_filter(struct section_buf *s)
{
...
static void
stop_filter(struct section_buf *s)
{
...
static void
add_filter(struct section_buf *s)
{
...
static void
remove_filter(struct section_buf *s)
{
    verbosedebug("%s: pid %d (0x%04x)\n", __FUNCTION__, s->pid, s->pid);
    stop_filter(s);
        ...
        if (done || time(NULL) > s->start_time + s->timeout) {
            if (s->run_once) {
                if (done)
                    ...
                    char const *intro = "        Info: no data from ";
                    ...
                    case TABLE_NIT_ACT:
                        ...
                        break;
                    case TABLE_NIT_OTH:
                        verbose("%sNIT(other) after %lld seconds\n", intro, (long long)s->timeout);
...
char const *
scantype_to_text(scantype_t scantype)
{
    switch (scantype) {
    case SCAN_CABLE:
        return "CABLE";
...
atsc_del_sys(fe_modulation_t modulation)
{
    switch (modulation) {
    case VSB_8:
    case VSB_16:
...
static void
copy_fe_params(struct transponder *dest, struct transponder *source)
{
    memcpy(&dest->frequency, &source->frequency, (void *)&source->private_from_here - (void *)&source->frequency);

    // NOTE: pointer calc in memcpy is 36 bytes actually.
}

static int
set_frontend(int frontend_fd, struct transponder *t)
{
    ...
    case SCAN_SATELLITE:
        ...
        if (scr_config.user_frequency > 0) {
            ...
            if (setup_scr(frontend_fd, t, &this_lnb, &scr_config))
                return -2;
            intermediate_freq = (scr_config.user_frequency + scr_config.offset) * 1000UL; // tune dvb card to users freq. NOTE:
                                                                                          // MHz -> kHz.
        ...
        if ((intermediate_freq < fe_info.frequency_min) || (intermediate_freq > fe_info.frequency_max)) {
            info(
                "\t skipped: (freq %.2f unsupported by driver: min=%.2f, max=%.2f)\n",
                intermediate_freq / 1e6,
...
uint16_t
carrier_timeout(uint8_t delsys)
{
    switch (delsys) {
    case SYS_DVBT:
    case SYS_DVBT2:
...
static int
tune_to_transponder(int frontend_fd, struct transponder *t)
{
...
static int
tune_to_next_transponder(int frontend_fd)
{
    struct transponder *t;
    uint8_t i, j;
...
static uint16_t
check_frontend(int fd, int verbose)
{
    fe_status_t status;
    ...
    if (verbose && !flags.emulate) {
        ...
        info("signal %04x | snr %04x | ber %08x | unc %08x | ", signal, snr, ber, uncorrected_blocks);
...
static unsigned int
chan_to_freq(int channel, int channellist)
{
...
uint16_t
fe_get_delsys(int frontend_fd, struct transponder *t)
{
 ...
 */
static bool
initial_table_lookup(int frontend_fd)
{
    struct section_buf s;
    int result;
                        ...
                        for (sr_parm = dvbc_symbolrate_min; sr_parm <= dvbc_symbolrate_max; sr_parm++) {
                            ...
                            for (plp_id_parm = plp_id_min; plp_id_parm <= plp_id_max; plp_id_parm++) {
                                ...
                                case SCAN_TERRCABLE_ATSC:
                                    ...
                                    if (is_known_initial_transponder(&test, 0)) {
                                        info(
                                            "%d %s: skipped (already known transponder)\n",
                                            freq_scale(f, 1e-3),
                                            atsc_mod_to_txt(this_atsc));
                                        continue;
                                    }
...
static void
network_scan(int frontend_fd, int tuning_data)
{
...
int
device_is_preferred(int caps, char const *frontend_name, uint16_t scantype)
{
...
int
get_api_version(int frontend_fd, struct w_scan_flags *flags)
{
...
void
bubbleSort(pList list, cmp_func compare)
{
    if (compare == NULL) {
...
static void
dump_lists(int adapter, int frontend)
{
    struct transponder *t;
    struct service *s;
    int n = 0, i, index = 0;
    ...
    for (t = scanned_transponders->first; t; t = t->next) {
        if (output_format == OUTPUT_DVBSCAN_TUNING_DATA && ((t->source >> 8) == TABLE_NIT_ACT)) {
            dvbscan_dump_tuningdata(dest, t, index++, &flags);
            continue;
        }
        if (output_format == OUTPUT_DVBV5SCAN_TUNING_DATA && ((t->source >> 8) == TABLE_NIT_ACT)) {
        ...
        for (s = (t->services)->first; s; s = s->next) {
            ...
            if (s->video_pid && !(serv_select & 1)) // vpid, this is tv
                continue; /* no TV services */
            if (!s->video_pid && (s->audio_num || s->ac3_num) && !(serv_select & 2)) // no vpid, but apid or ac3pid, this is
                                                                                     // radio
                continue; /* no radio services */
            if (!s->video_pid && !(s->audio_num || s->ac3_num) && !(serv_select & 4)) // no vpid, no apid, no ac3pid, this is
                                                                                      // service/other
                continue; /* no data/other services */
...
bool
fe_supports_scan(int fd, scantype_t type, struct dvb_frontend_info info)
{
    ...
    if (flags.api_version >= 0x0505) {
        ...
        for (; p[0].u.buffer.len > 0; p[0].u.buffer.len--) {
            ...
            char const *dname[] = { "UNDEFINED", "DVB-C ann.A", "DVB-C ann.B", "DVB-T",     "DSS",        "DVB-S",   "DVB-S2",
                                    "DVB-H",     "ISDB-T",      "ISDB-S",      "ISDB-C",    "ATSC",       "ATSC/MH", "DTMB",
                                    "CMMB",      "DAB",         "DVB-T2",      "TURBO-FEC", "DVB-C ann.C" };
...
#define cleanup() cl(country); cl(satellite); cl(initdata); cl(positionfile); cl(codepage);
    ...
    while ((opt = getopt_long(argc, argv, "5a:c:de:f:hi:l:o:p:qr:s:t:u:vxA:C:D:E:FGHI:LMO:PQ:R:S:T:VXZ", long_options, NULL)) !=
           -1) {
        switch (opt) {
        case 'a': // adapter
        ...
        case 'u': // SCR user definition: <slot>:<user frequency>:<SatPos 'A' or 'B'>(:<Pin>) "N:N:<A|B>(:Pin)"
            {
                ...
                if (sscanf(optarg, "%d:%d:%c:%d", &i1, &i2, &c, &i3) < 3)
                    bad_usage(argv[0]);
                scr_config.slot = i1;
    ...
    case SCAN_SATELLITE:
        ...
        if (scr_config.user_frequency)
            info(
                "SCR: slot=%u, userfreq=%uMHz, satpos=%c, pin=%d\n",
                scr_config.slot,
                scr_config.user_frequency,
                scr_config.pos == 1 ? 'B' : 'A',
                scr_config.pin <= 255 ? scr_config.pin : -1);
        break;
    default:
        cleanup();
    ...
    if (scantype == SCAN_TERRESTRIAL) {
        info(
            "scan type %s, delivery system %s, channellist %d\n",
            scantype_to_text(scantype),
            delivery_system_name(flags.delsys),
            this_channellist);
    } else {
        ...
        for (i = 0; i < DVB_ADAPTER_SCAN; i++) {
            for (j = 0; j < 4; j++) {
                ...
                if (fe_supports_scan(frontend_fd, scantype, fe_info)) {
                    ...
                    info(
                        "\t%s -> \"%s\" doesnt support %s -> SEARCH NEXT ONE.\n",
                        frontend_devname,
                        fe_info.name,
                        scantype_to_text(scantype));
                    close(frontend_fd);

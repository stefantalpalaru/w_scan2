...
enum __extflags { ignore, wscan_version, tuning_timeout, filter_timeout, fe_type, list_idx };
...
enum __rotor_file_formats { rotor_fileformat_undef, rotor_fileformat_wscan, rotor_fileformat_vdrpluginrotor };
 ...
 */
int
vdr_code_from_token(char const *token)
{
    int code = -1;
    int pos = 0;
    int dot = 0;
    ...
    while (*++token) {
        ...
        case 'W':
            ...
            break;
        default:
            fatal("%s: unknown source character '%c'", __FUNCTION__, *token);
        }
    }
    if (neg)
        pos |= st_Neg;
 ...
 */
void
vdr_source_to_str(int id, char *buffer, int bufsize)
{
    char *q = buffer;
    switch (id & st_Mask) {
    case stSat:
        *q++ = 'S';
        {
            int pos = id & ~st_Mask;
            if ((pos & ~st_Neg) % 10)
                q += snprintf(q, bufsize - 2, "%u.%u", (pos & ~st_Neg) / 10, (pos & ~st_Neg) % 10);
            else
                q += snprintf(q, bufsize - 2, "%u", (pos & ~st_Neg) / 10);
            *q++ = (id & st_Neg) ? 'E' : 'W';
        }
        break;
    default:

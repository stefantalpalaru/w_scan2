 ...
 */
int
choose_country(char const *country, int *atsc, int *dvb, uint16_t *scan_type, int *channellist)
{
 ...
 */
int
base_offset(int channel, int channellist)
{
    switch (channellist) {
    ...
    case DVBT_PA: // DVB-T Panama, 6 MHz step
        switch (channel) {
            // scan up to ch 60
        case 26 ... 60:
            return 389142857; // for accurate channel finding, this number needs to be exact according to ISDB-T center here: https://en.wikipedia.org/wiki/Television_channel_frequencies#Americas_(most_countries),_South_Korea,_Taiwan,_Burma_(Myanmar)_and_the_Philippines
        default:
            return SKIP_CHANNEL;
        }
 ...
 */
int
freq_step(int channel, int channellist)
{
    switch (channellist) {
    case ATSC_QAM:
    case ATSC_VSB:
...
int
bandwidth(int channel, int channellist)
{
 ...
 */
int
freq_offset(int channel, int channellist, int index)
{
    switch (channellist) {
    case USERLIST:
        return 0;
 ...
 */
int
dvbt_transmission_mode(int channel, int channellist)
{
    switch (channellist) {
 ...
 */
int
delsysloop_min(int channel, int channellist, uint16_t delsys)
{
    switch (delsys) {
    case SYS_UNDEFINED:
        switch (channellist) {
 ...
 */
int
delsysloop_max(int channel, int channellist, uint16_t delsys)
{
    switch (delsys) {
    case SYS_UNDEFINED:
        switch (channellist) {
 ...
 */
int
dvbc_qam_max(int channel, int channellist)
{
    switch (channellist) {
    case DVBC_FI:
        return 2; // QAM128
...
int
dvbc_qam_min(int channel, int channellist)
{
    switch (channellist) {
    case DVBC_FI:
    case DVBC_BR:
...
int
atsc_is_vsb(int atsc)
{
    return (atsc & ATSC_VSB);
}

int
atsc_is_qam(int atsc)
{
...
int
get_user_country(void)
{
    int const cats[] = { LC_CTYPE, LC_COLLATE, LC_MESSAGES };
    char *buf, *pch, *pbuf;
    ...
    for (cat = 0; cat < sizeof(cats) / sizeof(cats[0]); cat++) {
        ...
        if (!strncmp(buf, "POSIX", MIN(strlen(buf), 5)) || !(strncmp(buf, "en", MIN(strlen(buf), 2)) && !isalpha(buf[2])))
            continue;
...
int
plp_id_loop_min(int country_id)
{
    switch (country_id) {
    case AT: //      AUSTRIA
...
int
plp_id_loop_max(int country_id)
{
    switch (country_id) {
    case AT: //      AUSTRIA
        return 1;

 ...
 *
 * Copyright (C) 2006-2014 Winfried Koehler, 2021 Jean-Marc Wislez
 *
...
const xml_param xml_params[] = {
  ...
  {"symbolrate", {SYS_DVBS, SYS_DVBS2, SYS_DSS, SYS_TURBO,
      SYS_DVBC_ANNEX_A, SYS_DVBC_ANNEX_B, SYS_DVBC_ANNEX_C,  0}, NO_AUTO},
...
// int minor = id & 0xFF;
  ...
  else if (id == 0x4B19)  return "RCAS or RIDSYS cas";
...
char* safe_xml(const char* input, char* output) {
  ...
  while (*p != '\0' && i<STRING_BUFFER_SIZE) {
    if (*p < 32) {
      ...
      // key to be shifted
      switch (shift) {
      case 0xc3: if (*p >= 0x80 && *p <= 0x85) { output[i++] = 'A'; break; }
                 if (*p == 0x86) { output[i++] = 'A'; output[i++] = 'E'; break; }
                 if (*p == 0x87) { output[i++] = 'C'; break; }
                 if (*p >= 0x88 && *p <= 0x8b) { output[i++] = 'E'; break; }
                 if (*p >= 0x8c && *p <= 0x8f) { output[i++] = 'I'; break; }
                 if (*p == 0x91) { output[i++] = 'N'; break; }
                 if ((*p >= 0x92 && *p <= 0x96) || *p == 0x98) { output[i++] = 'O'; break; }
                 if (*p >= 0x99 && *p <= 0x9c) { output[i++] = 'U'; break; }
                 if (*p == 0x9d) { output[i++] = 'Y'; break; }
                 if (*p == 0x9f) { output[i++] = 's'; output[i++] = 's'; break; }
                 if (*p >= 0xa0 && *p <= 0xa5) { output[i++] = 'a'; break; }
                 if (*p == 0xa6) { output[i++] = 'a'; output[i++] = 'e'; break; }
                 if (*p == 0xa7) { output[i++] = 'c'; break; }
                 if (*p >= 0xa8 && *p <= 0xab) { output[i++] = 'e'; break; }
                 if (*p >= 0xac && *p <= 0xaf) { output[i++] = 'i'; break; }
                 if (*p == 0xb1) { output[i++] = 'n'; break; }
                 if ((*p >= 0xb2 && *p <= 0xb6) || *p == 0xb8) { output[i++] = 'o'; break; }
                 if (*p >= 0xb9 && *p <= 0xbc) { output[i++] = 'u'; break; }
                 if (*p == 0xbd || *p == 0xbf) { output[i++] = 'y'; break; }
                 output[i++] = '?';
                 break;
      case 0xc5: if (*p == 0x92) { output[i++] = 'O'; output[i++] = 'E'; break; }   
                 if (*p == 0x93) { output[i++] = 'o'; output[i++] = 'e'; break; } 
                 output[i++] = '?';
                 break;
    ...
    for (s = (t->services)->first; s; s = s->next) {
      if ((s->video_pid || s->audio_pid[0]) 
        ...
        printf( 
          "%s<name char256=\"%s\"/>\n",
          get_indent(indent),
          safe_xml(s->service_name, (char*)&buffer));
        printf( 
        ...
        if(s->teletext_pid) {
          fprintf(dest, "%s<stream type=\"6\" pid=\"%u\" description=\"teletext\"/>\n", get_indent(indent), s->teletext_pid);
        }
